defmodule GitGistClone.Comments do
  @moduledoc """
  ...
  alias GitGistClone.Comments.Comment
  ...
  def subscribe_comments(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(GitGistClone.PubSub, "user:#{key}:comments")
  end

  defp broadcast_comment(%Scope{} = scope, message) do
    key = scope.user.id

    Phoenix.PubSub.broadcast(GitGistClone.PubSub, "user:#{key}:comments", message)
  end
  ...
  """
  def create_comment(user, attrs, scopes \\ %{}) do
    user
    |>Ecto.build_assoc(:comments)
    |>Comment.changeset(attrs, scopes)
    |>Repo.insert()
  end
