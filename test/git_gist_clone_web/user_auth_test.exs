defmodule GitGistCloneWeb.UserAuthTest do
  use GitGistCloneWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias GitGistClone.Accounts
  alias GitGistClone.Accounts.Scope
  alias GitGistCloneWeb.UserAuth

  import GitGistClone.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, GitGistCloneWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      GitGistCloneWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        GitGistCloneWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
